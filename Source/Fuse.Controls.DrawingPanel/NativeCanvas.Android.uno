...
namespace Fuse.Controls.Native.Android
{
	...
	extern(ANDROID) internal class NativeCanvas : ICanvas
	{
		...
		{
			_bitmapCanvas.Draw(line, _pixelsPerPoint);
		}
		...
		{
			_bitmapCanvas.Draw(circle, _pixelsPerPoint);
		}
		...
		public void Draw(Bitmap bitmap, Rect rect)
		{
			...
			var size = rect.Size;
			_bitmapCanvas.DrawBitmap(bitmap.Handle, pos.X, pos.Y, size.X, size.Y);
		}
	...
	{
		public static void Clear(this Java.Object canvasHandle, float4 color) { Clear(canvasHandle, (int)Uno.Color.ToArgb(color)); }
		...
		public static void Draw(this Java.Object canvasHandle, Line line, float scale)
		{
			line = line.Scale(scale);
			var from = line.From;
			var to = line.To;
			DrawLine(canvasHandle, from.X, from.Y, to.X, to.Y, line.Width, (int)Uno.Color.ToArgb(line.Color));
		}

		public static void Draw(this Java.Object canvasHandle, Internal.Circle circle, float scale)
		{
			circle = circle.Scale(scale);
			var center = circle.Center;
			DrawCircle(canvasHandle, center.X, center.Y, circle.Radius, (int)Uno.Color.ToArgb(circle.Color));
		}
		...
		[Foreign(Language.Java)]
		public static void DrawBitmap(this Java.Object canvasHandle, Java.Object bitmapHandle, float x, float y, float w, float h)
		@{
			((Canvas)canvasHandle).drawBitmap((Bitmap)bitmapHandle, null, new Rect((int)Math.ceil(x), (int)Math.ceil(y), (int)Math.ceil(w), (int)Math.ceil(h)), null);
		@}
