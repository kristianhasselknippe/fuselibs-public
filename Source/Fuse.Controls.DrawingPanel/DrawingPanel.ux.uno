...
namespace Fuse.Controls
{
	...
	public class DrawingPanelBase : Panel, ICanvasViewHost
	{
		...
		public float StrokeWidth
		{
			...
			{
				_strokeWidth = value;
				_strokeWidthSquared = value * value;
			}
		}

		float _strokeWidthSquared = 10.0f * 10.0f;
		float StrokeWidthSquared
		{
			get { return _strokeWidthSquared; }
		}
		...
		void OnPointerMoved(object sender, PointerMovedArgs args)
		{
			...
			var line = new Line(_previousCoord, currentCoord, StrokeWidth, StrokeColor);
		...
		void OnPointerReleased(object sender, PointerReleasedArgs args)
		{
			...
			var stroke = _history.LastOrDefault();
			var currentCoord = WindowToLocal(args.WindowPoint);
			if (Vector.LengthSquared(_previousCoord - currentCoord) > StrokeWidthSquared)
			{
				var line = new Line(_previousCoord, currentCoord, StrokeWidth, StrokeColor);
				_history.LastOrDefault().Lines.Add(line);
				Canvas.Draw(line);
			...
			{
				var circle = new Internal.Circle(currentCoord, StrokeWidth * 0.5f, StrokeColor);
